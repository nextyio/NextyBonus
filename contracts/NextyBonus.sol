...
contract NextyBonus {
        ...
        uint256 public EX_BONUS_PERCENT; //not constant
        ...
        function setExtraBonusPercent(uint256 _percent) public onlyOwner {
            require((0 <= _percent) && (_percent <= 100 - FIXED_PERCENT));
            EX_BONUS_PERCENT = _percent;
            emit ChangePercentSuccess(_percent);
        }
        ...
        //need to re-factory 4 functions
        ...
        //override function to createLockedAmount with endTime
        function createFixedAmount(address _address, uint256 _fixedAmount, uint256 _endTime) private {
            LockAtribute memory _fixed = LockAtribute({
                startTime: _endTime,
                endTime: _endTime + LOCK_DURATION,
                value: _fixedAmount,
                percent: FIXED_PERCENT,
                lockStatus: StatusType.Locked
            });
        ...
        function createBonusAmount(address _address, uint256 _bonusAmount) private {
            LockAtribute memory _bonus = LockAtribute({
                ...
                endTime: now + LOCK_DURATION,
                value: _bonusAmount,
                percent: EX_BONUS_PERCENT,
                lockStatus: StatusType.Locked
            });
            bonusAmount[_address].push(_bonus);
        }
        
        //override function to createLockedAmount with startTime
        function createBonusAmount(address _address, uint256 _bonusAmount, uint256 _startTime) private {
            LockAtribute memory _bonus = LockAtribute({
        ...
        function createLockedAmount(address _address, uint256 _lockedAmount) public onlyOwner {
            ...
            uint256 _bonusAmount = _lockedAmount.mul(EX_BONUS_PERCENT).div(100);
            uint256 _totalBonusAmount = _fixedAmount + _bonusAmount;
            ...
            totalAmount = totalAmount.sub(_totalBonusAmount);
            lockedAmount[_address] = lockedAmount[_address].add(_totalBonusAmount);
            emit CreatedSuccess(_address, _totalBonusAmount);
        }
        
        //override function to createLockedAmount with startTime
        function createLockedAmount(address _address, uint256 _lockedAmount, uint256 _startTime) public onlyOwner {
            require(_lockedAmount <= totalAmount);
            ...
            createFixedAmount(_address, _fixedAmount, _startTime);
            createBonusAmount(_address, _bonusAmount, _startTime);
        ...
        function getFixedHistory(address _address, uint256 _id) public view 
        returns(uint256, uint256, uint256, uint256, bool, uint256){
            ...
            percent = fixedAmount[_address][_id].percent;
            lockStatus = fixedAmount[_address][_id].lockStatus;
            removeable = false;
            
            return (startTime, endTime, value, uint256(lockStatus), removeable, percent);
        }
        
        function getBonusHistory(address _address, uint256 _id) public view 
            returns(uint256, uint256, uint256, uint256, bool, uint256){
            ...
            percent = bonusAmount[_address][_id].percent;
